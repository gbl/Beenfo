...
public class BeenfoScreen extends Screen {
    ...
    BeenfoScreen(Object object, int honeyLevel, List<String> beeNames) {
        ...
        for (String beeName: beeNames) {
            if (!(beeName.isEmpty())) {
                this.beeNames.add(ITextComponent.Serializer.getComponentFromJson(beeName));
            } else {
                this.beeNames.add(ITextComponent.Serializer.getComponentFromJson(beeName));
            }            
        }
    ...
    @Override
    public void render(MatrixStack stack, int mouseX, int mouseY, float partialTicks) { // render
        ...
        this.minecraft.getTextureManager().bindTexture(TEXTURE);      // client
        this.blit(stack, x, y, 0, 0, 176, 30);                    // blit
        ...
        for (int i=0; i<beeNames.size(); i++) {
            if (beeNames.get(i) != null) {
                font.func_238422_b_(stack, beeNames.get(i).func_241878_f(), x+48, y+32+(i)*(30)+8, 0x000000);      //fontRenderer.draw
            }
        }
        setBlitOffset(200);                                                // setZLevel?
        itemRenderer.zLevel = 200.0F;                                    // itemRenderer
        for (int i=0; i<honeyLevel; i++) {
            itemRenderer.renderItemIntoGUI(honeyBottle, x+8+(i*18), y+8);
        }
        setBlitOffset(0);
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) { // keyPressed
        if (super.keyPressed(keyCode, scanCode, modifiers)) {
           return true;
        } else if (keyCode == GLFW.GLFW_KEY_ESCAPE || this.minecraft.gameSettings.keyBindInventory.matchesKey(keyCode, scanCode)) {
              this.minecraft.player.closeScreen();                    // client
              return true;
        }
        return false;
    }
